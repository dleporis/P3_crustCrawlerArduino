...
uint16_t DXL_P_GAIN[] = {850, 750, 850, 100, 100};
uint8_t DXL_I_GAIN[] = {0, 0, 0, 0, 0};
uint16_t DXL_D_GAIN[] = {100, 1600, 10, 10000, 10000};
...
int16_t DXL_GOAL_PWM[] = {200,250,200,200,200};// 200, 200, 200, 200}; //PWM limit range 0(0 [%]) ~ 885(100 [%])
...
void printStatus(){
  ...
  // DEBUG_SERIAL.print(dxl.readControlTableItem(PRESENT_VELOCITY, DXL_ID));
  // DEBUG_SERIAL.print("PWM:" );
  // DEBUG_SERIAL.print(dxl.readControlTableItem(PRESENT_PWM, DXL_ID)); 
}
...
void setup() {
  ...
  lcd.clear();
  lcd.print("Ready to recieve commands in 3 secs");
  delay(3000);
  lcd.clear();
...
//  }
  ...
  //while (DEBUG_SERIAL.available() > 0) {
    ...
    if (serialdata.charAt(0) == 'x'){
      ...
      //INVERSE KINEMATICS_______________________________
